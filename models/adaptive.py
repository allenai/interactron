...
from models.mlp_detector import MLPDetector
...
class AdaptiveModel(nn.Module):
    ...
    def forward(self, images, labels):
        ...
        for task_id in range(images.batch_size):
            ...
            for k in range(1):
                ...
                grad = torch.autograd.grad(
                    ll,  # model_losses["category_prediction_loss"],  # ll,
                    theta_task,
                    create_graph=True,
                    retain_graph=True,
                    allow_unused=True,
        ...
        # meta_loss = sum([sum([x[k]for x in losses]) for k in losses[0]])
        # meta_loss.backward()

        # set_parameters(self.detector, iter(theta))
        # self.detector.zero_grad()
        # self.detector.model.zero_grad()
        # self.learned_loss.zero_grad()
